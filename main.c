...
int main(void)
{
    uint16_t valor = 0; //definición de variables
    char c='5';
    //char b='a';
    Configurar_PLL(_10MHZ); //configuracion de la velocidad de reloj (función)
    Configurar_UART0(); //configuración del UART (función)
    ...
    Configurar_GPIO(); //configuración GPIO (función)
    //Homogeneidad();
    //printString("Holis Bolis");
    printChar(c); //envia una c 
    //printChar(b);
    printString("\n");
    // char *arr = readString(',');
    // printString(&arr[0]);
